...
| Symptom | Cause | Fix |
|---------|--------|-----|
| Sign in redirect error (`AADSTS50011`) | Redirect URI mismatch | `getMsalClient()` computes the redirect URI as `origin + pathname` of whichever page you signed in from — not just `/agents.html`. Every page that can initiate Entra sign-in needs its own exact-match entry under SPA Settings → **Authentication → Redirect URIs**: `/`, `/agents.html`, `/claude-ops.html`, `/flight-demo.html`, for both `http://localhost:5173` and the prod Vercel origin (8 entries total — `scripts/entra-setup.sh` registers all of them). This exact class of bug already happened once with Auth0's callback URLs in this project — see `docs/auth0-setup.md`'s equivalent troubleshooting row. |
