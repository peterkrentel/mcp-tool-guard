...
| UI key | Scope rights | Flight | Documents |
|--------|--------------|--------|-----------|
| `read_only` | `flights:read`, `docs:read` | Search/details | List/get/search |
| `booking` | + `flights:write`, `docs:write` | Book/modify | Publish |
| `admin` | + `flights:delete`, `docs:delete` | Cancel booking | Archive doc |

Signed-in users: assign the same permission strings in Auth0 (or map via roles). See [auth0-setup.md](auth0-setup.md).
