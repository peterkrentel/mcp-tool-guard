...
- BL-046
  priority: P2
  status: todo
  item: Formal client-type attribution for MCP calls (Claude Code vs browser GUI vs curl vs future harnesses)
  acceptance: Model `client type` as a per-request attribute parallel to `trace_id`/`session_id` — not baked into agent identity, since the same M2M credential can in principle be used from more than one harness and conflating "who" (agent/scopes) with "how" (which software issued the call) would complicate BL-003's idempotent-agent-reuse work. Concretely: a `X-Client-Type` header (e.g. `claude-code`, `browser-gui`, `curl`) sent by `headersHelper`/the browser SDK, persisted onto `PendingRequest` and audit entries the same way `trace_id` already is; supersedes the informal `cc-` trace-id-prefix convention from BL-037. Surface it as a filterable column/view in the `/agents.html` approval-queue panel so an approver can tell at a glance which kind of client originated a pending request, instead of needing to know to inspect the trace_id string. Consider whether a given `client_type` should imply a default for BL-045's `X-Wait-For-Approval` opt-in (e.g. `claude-code` implying wait-for-approval by default) rather than requiring both headers separately.
  owner: unassigned
  source: raised 2026-07-19 while reviewing the BL-045 implementation plan — noted there's no formal way to distinguish which software (not which credential) made an MCP call today, only the informal `cc-` trace-id prefix; also relevant to Chris Keen's "AI Proxy Engine" (Fast AI Initiative), which has an analogous per-app/per-client attribution need for LLM provider governance
- BL-044
  priority: P1
  status: todo
