...
- `create_booking` returns HTTP 202 + `pending_id` + `pending_poll_token` when write scope is absent
- Agent polls `GET /pending/:id` with `X-Pending-Token` until an operator approves/denies
...
# Option B: use a write-scope token to skip the approval flow
export AGENT_JWT=$(node -e "console.log(require('../../ui/public/demo-tokens.json').booking)")
