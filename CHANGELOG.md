...
- **Track 3 — Approval queue (complete end-to-end)** — `gateway/pending-store.ts`, `MCP_APPROVAL_QUEUE=true` gate, `202` pending response path, admin `/pending/*` resolve routes; **Gemini native function-calling** replaces text-parsing (reliable tool JSON); **agent auto-retry** on pending approval; audit decision type includes `"pending"`; full proof script in progress
