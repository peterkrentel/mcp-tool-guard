...
- **Claude Code prod demo doc** — `docs/claude-code-demo.md` documents driving the deployed Render guard proxy from Claude Code via a new `ghprod` MCP server entry and `scripts/claude-mcp-token-helper-prod-demo.sh` (a static pre-vended-token `headersHelper`, workaround for BL-048's missing `clientSecret`). Records a real end-to-end run: a `repo:read`-only agent's `create_or_update_file` call denied on scope, held pending via the BL-045 long-poll, approved live through the Claude Code ops view, and forwarded to a real GitHub commit — plus an explanation of the single-JSON-RPC-call mechanics behind how Claude Code invokes an MCP tool.
