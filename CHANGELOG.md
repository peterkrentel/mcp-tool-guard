...
- **`IdpAdapter` interface + `Auth0IdpAdapter` wrapper** — gateway/idp-adapter.ts wraps existing gateway/auth0-mgmt.ts/token-vendor.ts logic behind a new `IdpAdapter` interface (part of BL-020 implementation, Task 1).
- **MCP_IDP_PROVIDER selection with fail-closed validation** — `idpProviderIdFromEnv()` in gateway/env.ts reads and validates the provider choice (defaults to auth0, case-insensitive, throws on unknown provider); `buildIdpAdapter()` in gateway/idp-adapter.ts constructs the active adapter, failing loudly when a selected provider has no implementation yet (part of BL-020 implementation, Task 2).
- **`IdpAdapter` wired into the gateway proxy (BL-020, Tasks 3-4)** — `gateway/proxy-routes-agents-token.ts` now consumes an injected `idpAdapter: IdpAdapter` option instead of importing `createM2mAgent`/`deleteM2mAgent` from `auth0-mgmt.ts` directly or taking separate `tokenVendor`/`apiAudience` options; `gateway/proxy-server.ts` constructs the adapter at startup via `buildIdpAdapter(idpProviderIdFromEnv())` inside a fail-closed try/catch that calls `process.exit(1)` on a bad `MCP_IDP_PROVIDER` config instead of starting with a broken IdP. `/health` now reports `idp_provider`, `idp_management_configured`, and `idp_vending_configured` in place of the old `jwt_trust_enabled`/`auth0_mgmt_configured` flags. `docs/render-deploy.md` and `docs/guard-proxy.md` updated to match — those are the two docs describing the gateway proxy's own `/health` (the flight server's separate, unmodified Python `/health` in `auth0-setup.md`/`RELEASE.md`/`vercel-deploy.md` is unaffected).
