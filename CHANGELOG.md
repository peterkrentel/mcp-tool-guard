...
- **`azp`/`appid` M2M detection misclassified delegated Entra admin tokens (regression from batch B above)** — `azp` isn't exclusive to `client_credentials` tokens: Entra also stamps it on delegated (authorization-code+PKCE) access tokens issued to a public client, exactly what `ui/src/auth.ts`'s `loginWithEntra()` uses for human operator sign-in. Since `assertActiveM2mAgent()` runs unconditionally and the SPA's client ID is never a provisioned M2M agent, every Entra admin's delegated token was being rejected with "Agent revoked or deleted" before scopes were even checked — blocking `POST /agents`, `DELETE /agents/:clientId`, and any other `gateway:admin`-gated route under Entra. Fixed by gating the `azp`/`appid` check in `clientIdFromPayload()` on the absence of a delegated-shape `scp` claim, which Microsoft's app-only (`client_credentials`) tokens never carry but delegated tokens always do; Entra M2M tokens (`azp`, no `scp`, only `roles`) are unaffected and still get revocation-checked.
