...
- **Backlog: added BL-046** — new P2 item for formal client-type attribution (Claude Code vs browser GUI vs curl) as a per-request `X-Client-Type` header/attribute parallel to `trace_id`, superseding the informal `cc-` prefix convention — raised while reviewing the BL-045 implementation plan.
