...
- **Gateway agent tool loop** — GatewayAgent now feeds tool results back to LLM in agentic loop; tool errors no longer terminate conversation, allowing Gemini to respond, retry, or guide user; LLM errors (e.g., 503) caught gracefully and displayed instead of crashing
