...
- **Persistent local gateway KV fallback** — `gateway/kv.ts` now uses a file-backed JSON store in non-production when Render/Upstash KV env vars are absent, so local `/servers`, `/agents`, approval queue state, recent audit rows, and other gateway KV-backed data survive a dev-server restart without running Redis or any extra process. The local file defaults to `memory/gateway-local-kv.json` and can be overridden with `MCP_LOCAL_KV_FILE`.
